...
\begin{abstract}
Large vision-language models (VLMs) for remote sensing typically operate in a
single-pass inference regime, limiting their ability to handle queries that
benefit from progressive spatial attention or multi-step reasoning.  We present
\textbf{GeoAgent}, a confidence-gated tool-augmentation layer built atop
GeoChat \citep{kuckreja2024geochat}, a grounded VLM pre-trained on diverse
remote sensing data.  GeoAgent equips the model with four spatial tools---region
cropping, image rotation, contrast enhancement, and edge detection---and invokes
them selectively: an initial prediction is accepted if it falls within the known
class vocabulary; otherwise the confidence gate triggers a tool-assisted
re-query.  On Remote Sensing Visual Question Answering (RSVQA LR), GeoAgent
raises accuracy from 90.70\% to 92.29\% with no weight updates.  On aerial
scene classification, confidence-gated tool use matches the zero-shot baseline
(UCMerced: 84.38\%, AID: 71.90\%), confirming that tools are invoked only when
genuinely needed.  We further fine-tune GeoChat on UCMerced and AID via
parameter-efficient LoRA adaptation, achieving \textbf{97.14\%} (+12.66~pp) and
\textbf{95.71\%} (+23.81~pp) respectively---with the confidence gate becoming
redundant on fine-tuned models whose predictions are always within vocabulary.
Qualitative analysis on four unseen remote-sensing scenes shows the FT model
corrects hallucinations present in the zero-shot baseline.  Together, these
results demonstrate that confidence-gated tool augmentation and lightweight
domain adaptation are complementary strategies for improving remote sensing VLMs.
\end{abstract}
...
\begin{enumerate}
  ...
  \item Empirical evaluation on RSVQA LR and aerial scene classification
    (UCMerced, AID), demonstrating VQA gains with no additional
    training.
...
1{,}507 images for training and 1{,}493 images for testing.  Figure~\ref{fig:aid} illustrates two
...
\begin{table}[t]
  \caption{Dataset statistics used in our experiments.  For RSVQA LR the
    evaluation set is the 10{,}004-sample benchmark provided with GeoChat,
    which covers test questions from both presence and relational categories.
    RSVQA HR was not evaluated due to data availability constraints.
    UCMerced and AID use official 80/20 and 50/50 per-class splits,
    respectively; neither has an official validation split.}
  \label{tab:datasets}
  \centering
  \begin{tabular}{lrrrl}
    \toprule
    \textbf{Dataset} & \textbf{Train} & \textbf{Val} & \textbf{Test (Eval)} & \textbf{Task} \\
    \midrule
    RSVQA LR  & 64{,}833 & 7{,}714 & 10{,}004 & VQA \\
    RSVQA HR  & ---       & ---     & ---       & VQA (not eval.) \\
    UCMerced  & 1{,}680   & ---     & 420       & Classification \\
    AID       & 1{,}507   & ---     & 1{,}493   & Classification \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[t]
  \caption{Average accuracy (\%) on RSVQA LR.  GeoAgent improves
    over single-pass GeoChat without any additional training.  RSVQA HR was
    not evaluated due to benchmark data unavailability.}
  \label{tab:rsvqa}
  \centering
  \begin{tabular}{lc}
    \toprule
...
Table~\ref{tab:cls} reports top-1 accuracy on UCMerced and AID.  GeoAgent
with confidence-gated routing nearly matches the single-pass baseline on both
datasets (UCMerced: 84.38\% vs 84.48\%, AID: 71.90\%).  When the classification
prompt lists all valid class names explicitly, GeoChat reliably returns one of
them in the initial pass, so the confidence gate---which invokes contrast
enhancement only when the answer falls outside the known class vocabulary---triggers
for only 4.9\% of UCMerced queries and 0\% of AID queries.  For the small fraction
where the gate does activate, contrast enhancement yields negligible net benefit
on these well-lit, high-resolution datasets.

\begin{table}[t]
  \caption{Top-1 classification accuracy (\%) on UCMerced and AID.  GeoAgent
    with confidence-gated contrast enhancement nearly matches the single-pass
    baseline; the gate triggers for only 4.9\% of UCMerced queries and
    is never activated on AID.}
  \label{tab:cls}
  \centering
  \begin{tabular}{lcc}
    \toprule
    ...
    \midrule
    GeoChat (base)     & \textbf{84.48} & 71.90 \\
    GeoChat + GeoAgent & 84.38          & \textbf{71.90} \\
    \bottomrule
  \end{tabular}
\end{table}

The absence of tool invocation on classification benchmarks is consistent with
the design of the confidence gate: for structured tasks with a closed, explicitly
listed class set the model operates at high confidence by construction, and the
agent correctly delegates entirely to the base model.  The tool-augmented
reasoning capability is more beneficial for open-ended VQA tasks (e.g., RSVQA)
where the model must resolve spatial ambiguity across a large, unconstrained
answer space.
...
single NVIDIA RTX 4500 Ada Generation GPU (24\,GB VRAM) using LoRA (rank 64, $\alpha=128$); training
takes approximately 40 minutes per dataset.  The GeoAgent configuration is
identical to the zero-shot experiments ($T_{\max}=3$, all four tools
enabled, confidence gate active).

\subsection{Results}

Table~\ref{tab:finetune} reports accuracy after fine-tuning on UCMerced and AID.
Fine-tuning alone provides a substantial gain over the zero-shot GeoChat baseline
on both classification datasets.  Since fine-tuning was performed exclusively on
scene classification data, RSVQA performance is not re-evaluated for fine-tuned
models (\texttt{---} in the table).

\begin{table}[t]
  \caption{Accuracy (\%) after LoRA fine-tuning.  \emph{GeoChat FT} denotes
    single-pass inference with fine-tuned weights; \emph{GeoChat FT +
    GeoAgent} adds the confidence-gated agentic loop on top.  Zero-shot
    baselines from Tables~\ref{tab:rsvqa} and~\ref{tab:cls} are reproduced
    for reference.  RSVQA was not re-evaluated for fine-tuned models since
    training was performed on classification data only.}
  \label{tab:finetune}
  \centering
  \begin{tabular}{lccc}
    \toprule
    \textbf{Method} & \textbf{RSVQA LR (\%)} & \textbf{UCMerced (\%)} & \textbf{AID (\%)} \\
    \midrule
    GeoChat (zero-shot)            & 90.70 & 84.48 & 71.90 \\
    GeoChat + GeoAgent (zero-shot) & 92.29 & 84.38 & 71.90 \\
    \midrule
    GeoChat FT                     & ---   & \textbf{97.14} & \textbf{95.71} \\
    GeoChat FT + GeoAgent          & ---   & \textbf{97.14} & \textbf{95.71} \\
    \bottomrule
  \end{tabular}
\end{table}

Fine-tuning provides substantial gains over the zero-shot baselines on both
classification benchmarks.  After domain adaptation on UCMerced training data,
GeoChat FT achieves \textbf{97.14}\% on UCMerced, and GeoChat FT + GeoAgent
achieves \textbf{97.14}\%, compared to 84.48\% for the zero-shot baseline.
Similarly, fine-tuning on AID yields \textbf{95.71}\% for GeoChat FT and
\textbf{95.71}\% for GeoChat FT + GeoAgent, compared to 71.90\% at zero-shot.
We attribute these gains to the fine-tuned backbone developing more
task-calibrated visual representations optimised for the target class taxonomy.
On fine-tuned models, the confidence gate never triggers since domain-adapted
predictions consistently fall within the target class vocabulary, yielding
identical scores for GeoChat FT and GeoChat FT + GeoAgent.
...
answer from multi-step context.  Experiments on RSVQA LR and
aerial scene classification (UCMerced, AID) show that GeoAgent improves VQA
accuracy by 1.59 percentage points with no additional fine-tuning.  On scene
classification, confidence-gated routing correctly suppresses tool invocation
when GeoChat already operates with high confidence on closed-vocabulary tasks,
matching the single-pass baseline without any accuracy regression.
...
% ===========================================================================
\appendix
\section{Qualitative Inference Examples}
\label{app:qualitative}
% ===========================================================================

This appendix presents four qualitative examples on real-world remote-sensing
images not seen during training or evaluation.  For each image we show (i) the
original tile, (ii) the contrast-enhanced version produced by the
\texttt{enhance\_contrast} tool, (iii) the answer from the \emph{base}
GeoChat, and (iv) the step-by-step trace from \emph{GeoChat FT + GeoAgent}.
The GeoAgent applies contrast enhancement on every query as a demonstration
step; the \emph{confidence gate} (Section~\ref{sec:method}) did not trigger on
any of these images, meaning the fine-tuned model's initial prediction already
contained recognisable vocabulary and did not require a forced tool re-query.
All tool calls here are therefore demonstrative rather than remedial.

% ---------------------------------------------------------------------------
\subsection*{Example 1 — Agricultural Farmland with Roundabout}
% ---------------------------------------------------------------------------

\begin{figure}[h]
  \centering
  \begin{minipage}{0.48\linewidth}
    \includegraphics[width=\linewidth]{inference_imgs/image.png}
    \caption*{(a) Original image}
  \end{minipage}\hfill
  \begin{minipage}{0.48\linewidth}
    \includegraphics[width=\linewidth]{results/inference_demo/image_tool_enhance_contrast.png}
    \caption*{(b) After \texttt{enhance\_contrast} (factor = 1.5)}
  \end{minipage}
  \caption{Example 1: Agricultural farmland with a circular roundabout and
           orchard rows.}
  \label{fig:demo1}
\end{figure}

\noindent\textbf{Question:} What is the primary land use category of this
remote sensing image? Is the scene agricultural, commercial, residential, or
industrial?

\noindent\textbf{Ground truth:} Agricultural / farmland.

\medskip
\noindent\textbf{Base GeoChat:}
\begin{quote}\small
\textit{``Based on the information provided, it is difficult to determine the
primary land use category\ldots The presence of a basketball court and a
building suggests that the area is likely urban or suburban\ldots''}
\end{quote}
The base model fails to recognise the agricultural context and instead
hallucinates a basketball court.

\medskip
\noindent\textbf{GeoChat FT + GeoAgent trace:}
\begin{enumerate}[leftmargin=*, label=\textbf{Step \arabic*.}]
  \item \textbf{[initial — original image]}
    \textit{``The primary land use category is likely \textbf{agricultural},
    as there are multiple storage tanks\ldots commonly used in agricultural
    settings for storing and distributing water or other resources.''}
    Confidence gate: \emph{not triggered} (answer contains meaningful
    vocabulary).
  \item \textbf{[tool: \texttt{enhance\_contrast}]}
    Re-query on contrast-enhanced image.
    \textit{``The primary land use category is likely \textbf{agricultural}\ldots
    suggesting the area is likely used for agricultural purposes, such as
    irrigation or crop production.''}
    Answer \emph{unchanged} in classification; contrast enhancement
    slightly sharpens the description.
  \item \textbf{[final]}
    Final answer: \emph{agricultural}.  Correct land-use category despite
    the model misidentifying the roundabout as storage tanks.
\end{enumerate}

% ---------------------------------------------------------------------------
\subsection*{Example 2 — Football Pitches Adjacent to a River}
% ---------------------------------------------------------------------------

\begin{figure}[h]
  \centering
  \begin{minipage}{0.48\linewidth}
    \includegraphics[width=\linewidth]{inference_imgs/image2.png}
    \caption*{(a) Original image}
  \end{minipage}\hfill
  \begin{minipage}{0.48\linewidth}
    \includegraphics[width=\linewidth]{results/inference_demo/image2_tool_enhance_contrast.png}
    \caption*{(b) After \texttt{enhance\_contrast} (factor = 1.5)}
  \end{minipage}
  \caption{Example 2: Two football pitches adjacent to a river/lake.}
  \label{fig:demo2}
\end{figure}

\noindent\textbf{Question:} How many sports fields are visible in this image?
Describe their type and the surrounding environment.

\noindent\textbf{Ground truth:} Two football pitches next to a body of water.

\medskip
\noindent\textbf{Base GeoChat:}
\begin{quote}\small
\textit{``There are a total of \textbf{3} sports fields\ldots One is a football
field, one is a soccer field, and the third is a \textbf{ground track
field}\ldots''}
\end{quote}
The base model over-counts (3 instead of 2) and hallucinates a non-existent
athletics track.
...
\begin{enumerate}[leftmargin=*, label=\textbf{Step \arabic*.}]
  \item \textbf{[initial — original image]}
    \textit{``There are \textbf{two} sports fields visible\ldots One is a
    soccer field, and the other is a football field\ldots The surrounding
    environment consists of trees and a road.''}
    Correct count; confidence gate not triggered.
  \item \textbf{[tool: \texttt{enhance\_contrast}]}
    \textit{``There are \textbf{two} sports fields\ldots The surrounding
    environment consists of a mix of trees and buildings, with some roads
    and a \textbf{bridge} in the background.''}
    Contrast enhancement reveals additional context (bridge/road structure
    near the river).
  \item \textbf{[final]}
    Final answer: \emph{two football / soccer fields}.  Correct count and
    type; base model was wrong.
\end{enumerate}

% ---------------------------------------------------------------------------
\subsection*{Example 3 — Dense Commercial Downtown Area}
% ---------------------------------------------------------------------------

\begin{figure}[h]
  \centering
  \begin{minipage}{0.48\linewidth}
    \includegraphics[width=\linewidth]{inference_imgs/image3.png}
    \caption*{(a) Original image}
  \end{minipage}\hfill
  \begin{minipage}{0.48\linewidth}
    \includegraphics[width=\linewidth]{results/inference_demo/image3_tool_enhance_contrast.png}
    \caption*{(b) After \texttt{enhance\_contrast} (factor = 1.5)}
  \end{minipage}
  \caption{Example 3: Dense commercial downtown with a large office complex.}
  \label{fig:demo3}
\end{figure}

\noindent\textbf{Question:} What type of urban area is shown? Classify the
land use of the central building complex visible in the image.

\noindent\textbf{Ground truth:} Commercial / institutional urban area with
large office buildings.

\medskip
\noindent\textbf{Base GeoChat:}
\begin{quote}\small
\textit{``The image shows a densely populated urban area with a mix of
residential and commercial buildings.  The central building complex is likely
a \textbf{commercial or mixed-use area}\ldots''}
\end{quote}
Reasonable classification; the model correctly identifies the commercial
character but does not distinguish the institutional scale of the complex.

\medskip
\noindent\textbf{GeoChat FT + GeoAgent trace:}
\begin{enumerate}[leftmargin=*, label=\textbf{Step \arabic*.}]
  \item \textbf{[initial — original image]}
    \textit{``\ldots likely a \textbf{city center or downtown district}.
    The central building complex may serve as a hub for \textbf{business,
    government, or cultural activities}\ldots''}
    More precise: identifies governmental / institutional function.
  \item \textbf{[tool: \texttt{enhance\_contrast}]}
    Answer unchanged in substance; contrast enhancement adds no new
    semantic information for this high-contrast urban tile.
  \item \textbf{[final]}
    Final answer: \emph{city center / downtown, commercial and institutional
    use}.  Comparable to the base model with higher specificity.
\end{enumerate}

% ---------------------------------------------------------------------------
\subsection*{Example 4 — Medium-Density Residential Area with Dome}
% ---------------------------------------------------------------------------

\begin{figure}[h]
  \centering
  \begin{minipage}{0.48\linewidth}
    \includegraphics[width=\linewidth]{inference_imgs/image4.png}
    \caption*{(a) Original image}
  \end{minipage}\hfill
  \begin{minipage}{0.48\linewidth}
    \includegraphics[width=\linewidth]{results/inference_demo/image4_tool_enhance_contrast.png}
    \caption*{(b) After \texttt{enhance\_contrast} (factor = 1.5)}
  \end{minipage}
  \caption{Example 4: Medium-density residential area with a circular dome
           structure (mosque).}
  \label{fig:demo4}
\end{figure}

\noindent\textbf{Question:} What type of residential environment is depicted?
Can you identify any notable structure in the scene?

\noindent\textbf{Ground truth:} Medium-density residential area with a dome
structure (mosque).

\medskip
\noindent\textbf{Base GeoChat:}
\begin{quote}\small
\textit{``The scene depicts a residential environment with a mix of buildings
and houses, as well as a roundabout and a road.  One notable structure is a
building located at the centre\ldots''}
\end{quote}
Identifies the residential character but does not recognise the dome / mosque.

\medskip
\noindent\textbf{GeoChat FT + GeoAgent trace:}
\begin{enumerate}[leftmargin=*, label=\textbf{Step \arabic*.}]
  \item \textbf{[initial — original image]}
    \textit{``The scene depicts a \textbf{residential environment} with a mix
    of buildings and trees\ldots The notable structure is the building at the
    centre, which appears to be a prominent feature in the landscape.''}
    Correct scene type; dome not named explicitly.
  \item \textbf{[tool: \texttt{enhance\_contrast}]}
    Answer unchanged; the dome structure remains unlabelled in both passes,
    indicating a limitation of the current model on religious architecture.
  \item \textbf{[final]}
    Final answer: \emph{residential area}.  Partial credit — scene type
    correct, specific structure (mosque dome) not identified by either model.
\end{enumerate}

\bigskip
\noindent\textbf{Summary.}
Table~\ref{tab:demo_summary} compares base GeoChat and GeoChat FT + GeoAgent
across all four examples.

\begin{table}[h]
  \caption{Qualitative comparison on four inference images.}
  \label{tab:demo_summary}
  \centering
  \small
  \begin{tabular}{p{2.8cm} p{5cm} p{5cm} c}
    \toprule
    \textbf{Image} & \textbf{Base GeoChat} & \textbf{GeoChat FT + GeoAgent} & \textbf{Confidence Gate Triggered?} \\
    \midrule
    Farmland / roundabout
      & Misclassifies scene; hallucinates basketball court
      & Correctly identifies agricultural use (storage tank confusion remains)
      & No \\[4pt]
    Football pitches
      & Over-counts (3 fields); hallucinates athletics track
      & Correct count (2) and field types; contrast reveals bridge context
      & No \\[4pt]
    Commercial downtown
      & Commercial / mixed-use (correct but vague)
      & City centre with business / government function (more specific)
      & No \\[4pt]
    Residential + dome
      & Residential scene; dome not identified
      & Residential scene; dome not identified (shared limitation)
      & No \\
    \bottomrule
  \end{tabular}
\end{table}
